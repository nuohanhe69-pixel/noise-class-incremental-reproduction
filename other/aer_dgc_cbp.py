...
class ErAceAerAbsOGC(ErAceAerAbs): # <<<--- IMPORTANT: Inherit from ErAceAerAbs
    ...
    def get_parser(parser) -> ArgumentParser:
        ...
        ogc_group.add_argument('--ogc_buffer_penalty_coeff', default=1, type=float,
    ...
    def apply_sap(self, dataset):
        ...
        # 2. Get all buffer data
        all_buf_data = self.buffer.get_all_data(device="cpu")
        inputs, labels = all_buf_data[0], all_buf_data[1]

        # Use CustomDataset to create a DataLoader for buffer data
        not_aug_dataset = CustomDataset(inputs, labels, transform=self.normalization_transform)
        not_aug_loader = DataLoader(not_aug_dataset,
                                    batch_size=self.args.batch_size,
                                    shuffle=False)

        # Split data using the model's inherited split_data (GMM on losses)
        # This will identify clean and ambiguous (noisy) samples based on their loss distribution
        ...
        clean_idxs = np.setdiff1d(all_idxs, amb_idxs) # Clean samples are those not identified as ambiguous

        # 3. Handle edge cases where no clean or no ambiguous samples are found
        if len(clean_idxs) == 0:
            print("No clean samples found in buffer for SAP. Skipping SAP Unlearning.")
            return
        if len(amb_idxs) == 0:
            print("No ambiguous (noisy) samples found in buffer for SAP. Skipping SAP Unlearning.")
            return
        
        # 4. SAP Execution safeguard (防止秩坍缩)
        min_required_samples = max(10, self.num_classes) # At least num_classes samples needed for SVD stability
        if len(clean_idxs) < min_required_samples:
            print(f"Too few clean samples ({len(clean_idxs)} < {min_required_samples}) to establish SVD protection space. Skipping SAP.")
            return

        # 5. Dynamically adjust and truncate sample sizes
        ...
        # Truncate extremely large clean sets to prevent SVD computation OOM
        if len(clean_idxs) > sap_retain_samples:
            import random
            random.shuffle(clean_idxs) # Randomly shuffle to ensure class balance
            clean_idxs = clean_idxs[:sap_retain_samples]

        print(f"SAP Execution: {len(clean_idxs)} Clean (Retain Space), {len(amb_idxs)} Noisy (Forget Space).")

        # 6. Build Datasets for SAP
        ...
        full_dataset = SapTestDataset(all_buf_inputs, all_buf_labels, transform=self.normalization_transform)
    ...
    def end_task(self, dataset):
        ...
        if self._current_task == self.N_TASKS - 1:
            ...
            # It seems the buffer might store original, noisy labels. Let's get true labels if available.
            # This is a guess; we assume the 'true_label' attribute exists if noise was added.
            if hasattr(self.buffer, 'true_labels'):
                buf_true_labels = self.buffer.true_labels
            else:
                # If not, assume buffer labels are the true labels
            ...
            self.net.eval()
            with torch.no_grad():
                # Assuming self.net.features() exists to extract features
                if hasattr(self.net, 'features'):
                    features = self.net.features(buf_inputs).cpu().numpy()
                else: # Fallback: use the whole network output before classifier
