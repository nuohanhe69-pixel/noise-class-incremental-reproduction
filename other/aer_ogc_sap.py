...
from utils.tsne_plotter import plot_tsne # for t-SNE visualization
import os
...
class OptimizedGradientClippingBase(nn.Module):
    def __init__(self, epsilon=0.1, time_frame_s=100, queue_size_q=1000,
                 binary_search_tol=1e-4, binary_search_max_iter=20, lazy_update_freq=10):
        super().__init__()
        ...
        # tau信息
        self.lr_t = 1.0 # This will be updated by the optimizer's LR
        self.prob_thr_t = self.min_prob
        self.tau_t = self.mapping_p_to_grad(torch.tensor(self.prob_thr_t)).abs().item()
        
        # Split Queues & Lazy Update State
        self.current_queue_H = deque(maxlen=queue_size_q)
        self.buffer_queue_H = deque(maxlen=queue_size_q)
        ...
        # Persistent GMM with warm_start for performance
        self.gmm = GaussianMixture(n_components=2, max_iter=200, tol=1e-3, reg_covar=1e-4, n_init=20, warm_start=True, init_params='kmeans')

    def _fit_gmm_internal(self, data):
        """Internal method to fit GMM using persistent instance"""
        try:
            self.gmm.fit(data)
        except Exception:
            # Fallback if warm start fails
            self.gmm = GaussianMixture(n_components=2, max_iter=200, tol=1e-3, reg_covar=1e-4, n_init=10, warm_start=True)
            self.gmm.fit(data)

        mean_index = np.argsort(self.gmm.means_, axis=0)
        mean_clean = self.gmm.means_[mean_index[0]][0]
        covar_clean = self.gmm.covariances_[mean_index[0]][0][0]
        mean_noisy = self.gmm.means_[mean_index[1]][0]
        covar_noisy = self.gmm.covariances_[mean_index[1]][0][0]
        
        return mean_clean, covar_clean, mean_noisy, covar_noisy
    ...
    def get_tau_t(self, current_lr, is_buffer=False, epsilon_mult=1.0):
        self.lr_t = current_lr # Update lr_t with current learning rate
        ...
        if len(queue) < self.queue_size_q:
            if is_buffer: return self.prob_thr_buffer, self.tau_buffer if self.tau_buffer else self.tau_t
            else: return self.prob_thr_current, self.tau_current if self.tau_current else self.tau_t

        queue_H = np.array(list(queue)).reshape(-1, 1)
        mean_clean, covar_clean, mean_noisy, covar_noisy = self._fit_gmm_internal(queue_H)
        ...
        # 计算概率密度
        # Ensure scale is not zero for stats.norm.pdf
        ...
        # Normalize to prevent division by zero or very small numbers
        p_H_clean_sum = p_H_clean_vals.sum()
        p_H_noisy_sum = p_H_noisy_vals.sum()
        self.p_H_clean = p_H_clean_vals / (p_H_clean_sum + 1e-9)
        self.p_H_noisy = p_H_noisy_vals / (p_H_noisy_sum + 1e-9)

        # 计算tau_t
        # Apply epsilon multiplier (warm-up)
        target_val = 1 + self.lr_t * self.epsilon * epsilon_mult
        
        prob_thr_t = binary_search(self.min_prob, self.max_prob,
                                   self.calc_grad_ratio, target_val,
    ...
    def calc_grad_ratio(self, prob):
        ...
        # Avoid division by zero if means are very small
        ...
        if e_grad_clean_mean < 1e-9: # Prevent division by zero
            return float('inf')

        grad_ratio = e_grad_noisy_mean / e_grad_clean_mean
        return grad_ratio

    def reset_queues_and_gmm(self):
        """
        Resets the queues and re-initializes the GMM and related state.
        """
        self.current_queue_H.clear()
        self.buffer_queue_H.clear()
        
        # Reset GMM to its initial state
        self.gmm = GaussianMixture(n_components=2, max_iter=200, tol=1e-3, reg_covar=1e-4, n_init=20, warm_start=True, init_params='kmeans')

        # Reset lazy update counters and stored tau values
        self.tau_current = None
        self.tau_buffer = None
        self.prob_thr_current = self.min_prob
        self.prob_thr_buffer = self.min_prob
    ...
    def forward(self, logits, labels, lr, tau=None):
        ...
        p_y_hat = prob[torch.arange(logits.shape[0]), labels] # Probability of the true class

        # Calculate the gradient of the loss w.r.t. p_y_hat
        # This is a conceptual gradient for the clipping, not the actual backprop
        ...
        return self.mapping_p_to_loss(p_y_hat).mean()
...
class ErAceAerAbsOGC(ErAceAerAbs): # <<<--- IMPORTANT: Inherit from ErAceAerAbs
    ...
    def get_parser(parser) -> ArgumentParser:
        ...
        ogc_group.add_argument('--ogc_queue_size', default=4096, type=int,
        ...
        ogc_group.add_argument('--ogc_low_conf_weight', default=0.5, type=float,
                            help='Weight assigned to low confidence (noisy) samples by OGC')
        ogc_group.add_argument('--ogc_buffer_penalty_coeff', default=2.0, type=float,
        ...
        sap_group.add_argument('--sap_retain_samples', type=int, default=2000, 
        ...
        sap_group.add_argument('--sap_end_layer', nargs='+', type=int, default=[3], 
                            ...
                            #    默认之前为 0  - 1；
        sap_group.add_argument('--sap_scale_coff', nargs='+', type=float, default=[500], # 针对CIL场景大幅下调，避免过度修正旧知识
    ...
    def apply_sap(self, dataset):
        """
        Apply Selective Activation Projection (SAP) to unlearn noisy samples in the buffer.
        """
        if self.buffer.is_empty():
            return

        # 动态计算 retain_samples：取 (配置值) 和 (当前 Buffer 大小的一半) 的最小值
        # 确保 Buffer 里剩下的另一半能作为 "corrupt" 集进行对比
        current_buf_size = len(self.buffer)
        retain_samples_count = min(self.args.sap_retain_samples, current_buf_size // 2)
    
        if retain_samples_count < self.dataset.N_CLASSES: # 样本太少不足以分配到每类
            print("Buffer samples too few for balanced SAP, skipping...")
            return

        # Get class-balanced data for the "clean" set
        # The get_balanced_data method returns (inputs, labels, indices)
        # We need to pass the transform to ensure consistency with other datasets
        # self.dataset.N_CLASSES should provide the total number of classes
        inputs_clean, labels_clean, clean_indices_buffer = self.buffer.get_balanced_data(
            size=retain_samples_count,
            transform=self.normalization_transform,
            n_classes=self.dataset.N_CLASSES # Use the total number of classes from the dataset
        )

        # If no clean samples are found (e.g., buffer is empty or size is too large for current buffer)
        if len(inputs_clean) == 0:
            return

        # 2. Get all buffer data to determine the 'corrupt' set
        # Assuming get_all_data returns examples, labels, and indices if available, otherwise compose indices
        all_buf_data = self.buffer.get_all_data(device="cpu")
        all_buf_inputs, all_buf_labels = all_buf_data[0], all_buf_data[1]
        
        # Create a tensor of all indices for the buffer
        all_buf_indices_full = torch.arange(len(all_buf_inputs))
        
        # Identify indices that are NOT in clean_indices_buffer for the corrupt set
        # Convert clean_indices_buffer to a set for efficient lookup
        clean_indices_set = set(clean_indices_buffer.tolist()) if isinstance(clean_indices_buffer, torch.Tensor) else set(clean_indices_buffer)
        
        corrupt_indices_list = [idx.item() for idx in all_buf_indices_full if idx.item() not in clean_indices_set]
        
        if len(corrupt_indices_list) == 0:
            # If there are no corrupt samples, SAP cannot be applied as there's nothing to "forget"
            return
        
        corrupt_indices = torch.tensor(corrupt_indices_list, device="cpu")
        inputs_corrupt = all_buf_inputs[corrupt_indices]
        labels_corrupt = all_buf_labels[corrupt_indices]

        # 3. Build datasets
        clean_dataset = CustomDataset(inputs_clean, labels_clean, transform=self.normalization_transform)
        corrupt_dataset = CustomDataset(inputs_corrupt, labels_corrupt, transform=self.normalization_transform)
        full_dataset = CustomDataset(all_buf_inputs, all_buf_labels, transform=self.normalization_transform)
    ...
    def split_data(self, test_loader: DataLoader, model: nn.Module):
        ...
        # Use more robust parameters than the base implementation
    ...
    def end_epoch(self, epoch: int, dataset: ContinualDataset):
        super().end_epoch(epoch, dataset) # Call parent (ErAceAerAbs) end_epoch (handles AER checkpoint saving)
        ...
        if self.args.enable_sap and self.args.sap_frequency_epochs > 0:
            if (epoch + 1) % self.args.sap_frequency_epochs == 0:
                ...
                self.apply_sap(dataset)

    def end_task(self, dataset):
        super().end_task(dataset) # Call parent end_task

        # t-SNE Visualization at the end of the last task
        if self._current_task == self.N_TASKS - 1:
            print(f"Performing t-SNE visualization for ErAceAerAbsOGC (Task {self._current_task + 1}/{self.N_TASKS})")
            features, labels = self.get_buffer_features_and_labels()
            save_dir = "output/tsne_plots"
            
            # Determine filename and title based on SAP enablement
            if self.args.enable_sap:
                sap_status = "with_SAP"
                title_suffix = " with SAP Enabled"
            else:
                sap_status = "without_SAP"
                title_suffix = " without SAP Enabled"
            
            save_path = os.path.join(save_dir, f"aer_ogc_sap_{sap_status}_task_{self._current_task + 1}.png")
            plot_tsne(features, labels, save_path, title=f"ErAceAerAbsOGC - Task {self._current_task + 1} Buffer Features{title_suffix}")
        
        # Reset OGC queues and GMM at the end of a task (start of next)
        if hasattr(self, 'ogc_loss_fn') and self.ogc_loss_fn is not None:
            self.ogc_loss_fn.reset_queues_and_gmm()
            print("OGC queues and GMM reset for new task.")
        
        # Reset epoch counter for task start detection in observe
        self.last_epoch = -1
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch, true_labels=None):
        ...
        # We use a heuristic: if epoch is 0 and we haven't reset for this task-epoch-0 sequence yet.
        # But observe is called many times. We need a way to know if we are at the very start.
        # Mammoth resets epoch to 0 for each task.
        # We can track the previous epoch.
        if not hasattr(self, 'last_epoch'):
            self.last_epoch = -1
        
        # If epoch drops to 0 from a higher number, or it's the very first 0 (last_epoch=-1), it's a new task.
        # Exception: if it's 0 and last_epoch is 0, it's just the next batch in epoch 0.
        if epoch == 0 and (self.last_epoch != 0):
             # Reset Current Queue at start of task to avoid distribution lag
             self.ogc_loss_fn.current_queue_H.clear()
             # Buffer Queue might also need clearing if distribution shifts significantly,
             # but keeping it might be safer for stability? 
             # Request says: "Task Switch 时清空一半".
             # Let's clear both to be safe and avoid "distribution lag" mentioned.
        ...
        # Combine CE and OGC loss for current data
        # We can apply ogc_weights_current here to CE loss, or just use the combined loss
        # The original OGC idea is to modify gradients, but here we're combining losses
        # and using weights for buffer selection.
        # Let's apply ogc_weights to CE for a more direct effect on noisy samples in current batch.
