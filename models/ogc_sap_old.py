...
# from utils.tsne_data_saver import save_tsne_data # for t-SNE data saving
import os
...
class OptimizedGradientClippingBase(nn.Module):
    def __init__(self, 
        ...
        self.history_prob_thr_current = [] 
        self.history_prob_thr_buffer = [] 
    ...
    def get_tau_t(self, current_lr, is_buffer=False, epsilon_mult=1.0, save_gmm_path=None): 
        if self.use_single_queue: 
            is_buffer = False 
        ...
        if is_buffer: 
            ...
            if self.update_count_buffer % self.lazy_update_freq != 0 and self.tau_buffer is not None and save_gmm_path is None: 
                return self.prob_thr_buffer, self.tau_buffer 
        else: 
            self.update_count_current += 1 
            if self.update_count_current % self.lazy_update_freq != 0 and self.tau_current is not None and save_gmm_path is None: 
        ...
        if save_gmm_path: 
            os.makedirs(os.path.dirname(save_gmm_path), exist_ok=True) 
            gmm_data = { 
                'queue_H': queue_H.flatten(), 
                'mean_clean': mean_clean, 
                'covar_clean': covar_clean, 
                'mean_noisy': mean_noisy, 
                'covar_noisy': covar_noisy, 
                'weights': gmm_weights,
                'queue_mode': 'single' if self.use_single_queue else 'dual',
                'data_source': 'buffer' if is_buffer else 'current',
                'run_id': self.run_id,
            } 
            np.save(save_gmm_path, gmm_data) 
            print(f"Saved GMM visualization data to {save_gmm_path}") 
        ...
        if is_buffer: 
            ...
            self.history_prob_thr_buffer.append(prob_thr_t) 
        else: 
            self.prob_thr_current = prob_thr_t 
            self.tau_current = tau_t 
            self.history_prob_thr_current.append(prob_thr_t) 
...
class ErAceAerAbsOGC(ErAceAerAbs): # <<<--- IMPORTANT: Inherit from ErAceAerAbs
    ...
    def get_parser(parser) -> ArgumentParser:
        ...
        ogc_group.add_argument('--ogc_epsilon', default=15, type=float,
        ...
        ogc_group.add_argument('--ogc_loss_weight', default=1.0, type=float,
        ...
        # ogc_group.add_argument('--disable_ogc_buffer_filtering', type=int, default=0,
        #                     help='Ablation: Use standard loss for buffer insertion (no OGC filtering)')
        ogc_group.add_argument('--use_single_queue', type=int, default=0,
                            help='Ablation: Use a single queue for both current and buffer data (no decoupling)')
        ogc_group.add_argument('--ogc_run_id', type=str, default='default',
                            help='Unique run ID for experiment logs to avoid overwriting')
        ...
        sap_group.add_argument('--sap_retain_samples', type=int, default=1500, # Updated default for CIFAR10 buffer 500 scenario
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        ogc_kwargs = {
            ...
            # 'disable_gradient_clipping': bool(args.disable_ogc_gradient_clipping),
            'use_single_queue': bool(args.use_single_queue),
            'run_id': args.ogc_run_id,
        ...
        self.constraint_log_dir = self._get_mode_output_dir("/home/supermicro/LJW/mam/mammoth/output/constraint_logs/")
        os.makedirs(self.constraint_log_dir, exist_ok=True)
        self._reset_constraint_stats()

    def _get_queue_mode_suffix(self):
        return "single_queue" if self.args.use_single_queue else "dual_queue"

    def _get_plot_mode_suffix(self):
        return "single" if self.args.use_single_queue else "dual"

    def _get_mode_output_dir(self, base_dir):
        return os.path.join(base_dir, self._get_queue_mode_suffix())

    def _reset_constraint_stats(self):
        self.constraint_stats = {
            'current_total': 0,
            'current_noisy_total': 0,
            'current_low_conf_total': 0,
            'current_low_conf_noisy': 0,
            'current_clipped_total': 0,
            'current_clipped_noisy': 0,
            'buffer_total': 0,
            'buffer_noisy_total': 0,
            'buffer_low_conf_total': 0,
            'buffer_low_conf_noisy': 0,
            'buffer_clipped_total': 0,
            'buffer_clipped_noisy': 0,
        }

    def _update_constraint_stats(self, prefix, noisy_mask, low_conf_mask, clipped_mask):
        if noisy_mask is None:
            return

        noisy_mask = noisy_mask.detach().bool()
        low_conf_mask = low_conf_mask.detach().bool()
        clipped_mask = clipped_mask.detach().bool()

        self.constraint_stats[f'{prefix}_total'] += int(noisy_mask.numel())
        self.constraint_stats[f'{prefix}_noisy_total'] += int(noisy_mask.sum().item())
        self.constraint_stats[f'{prefix}_low_conf_total'] += int(low_conf_mask.sum().item())
        self.constraint_stats[f'{prefix}_low_conf_noisy'] += int((low_conf_mask & noisy_mask).sum().item())
        self.constraint_stats[f'{prefix}_clipped_total'] += int(clipped_mask.sum().item())
        self.constraint_stats[f'{prefix}_clipped_noisy'] += int((clipped_mask & noisy_mask).sum().item())

    def _safe_ratio(self, numerator, denominator):
        return float(numerator) / float(denominator) if denominator > 0 else None

    def _save_constraint_stats(self, task_id):
        suffix = self._get_queue_mode_suffix()
        run_id = self.args.ogc_run_id
        stats = dict(self.constraint_stats)

        stats.update({
            'run_id': run_id,
            'task_id': int(task_id),
            'queue_mode': 'single' if self.args.use_single_queue else 'dual',
            'current_low_conf_precision': self._safe_ratio(stats['current_low_conf_noisy'], stats['current_low_conf_total']),
            'current_low_conf_recall': self._safe_ratio(stats['current_low_conf_noisy'], stats['current_noisy_total']),
            'current_clipped_precision': self._safe_ratio(stats['current_clipped_noisy'], stats['current_clipped_total']),
            'current_clipped_recall': self._safe_ratio(stats['current_clipped_noisy'], stats['current_noisy_total']),
            'buffer_low_conf_precision': self._safe_ratio(stats['buffer_low_conf_noisy'], stats['buffer_low_conf_total']),
            'buffer_low_conf_recall': self._safe_ratio(stats['buffer_low_conf_noisy'], stats['buffer_noisy_total']),
            'buffer_clipped_precision': self._safe_ratio(stats['buffer_clipped_noisy'], stats['buffer_clipped_total']),
            'buffer_clipped_recall': self._safe_ratio(stats['buffer_clipped_noisy'], stats['buffer_noisy_total']),
        })

        save_path = os.path.join(
            self.constraint_log_dir,
            f"run_{run_id}_task_{task_id}_{suffix}_constraint_stats.json"
        )
    ...
    def end_task(self, dataset): 
        ...
        if self._current_task == self.N_TASKS - 1: 
            print(f"Saving data for t-SNE visualization (Task {self._current_task + 1}/{self.N_TASKS})") 
    
            if self.buffer.is_empty(): 
                print("Buffer is empty. Skipping t-SNE data saving.") 
            else: 
                all_buf_data = self.buffer.get_all_data(device=self.device) 
                buf_inputs, buf_labels = all_buf_data[0], all_buf_data[1] 
    
                if hasattr(self.buffer, 'true_labels'): 
                    buf_true_labels = self.buffer.true_labels 
                else: 
                    buf_true_labels = buf_labels 
    
                was_training = self.net.training 
                self.net.eval() 
                with torch.no_grad(): 
                    if hasattr(self.net, 'features'): 
                        features = self.net.features(buf_inputs).cpu().numpy() 
                    else: 
                        features = self.net(buf_inputs, returnt='features').cpu().numpy() 
                self.net.train(was_training) 
    
                true_labels_to_save = buf_true_labels.cpu().numpy() 
                noisy_labels_to_save = buf_labels.cpu().numpy() 
    
                save_dir = "/home/supermicro/LJW/mam/mammoth/output/tsne_data1/" 
                save_dir = self._get_mode_output_dir(save_dir)
                os.makedirs(save_dir, exist_ok=True) 
    
                np.save(os.path.join(save_dir, 'SAP_features.npy'), features) 
                np.save(os.path.join(save_dir, 'SAP_true_labels.npy'), true_labels_to_save) 
                np.save(os.path.join(save_dir, 'SAP_noisy_labels.npy'), noisy_labels_to_save) 
    
                print(f"Successfully saved features, true_labels, and noisy_labels to {save_dir}") 
    
        save_dir = self._get_mode_output_dir("/home/supermicro/LJW/mam/mammoth/output/ablation_logs/") 
        os.makedirs(save_dir, exist_ok=True) 
        task_id = self._current_task 
        suffix = self._get_queue_mode_suffix() 
        run_id = self.args.ogc_run_id 
    
        np.save(os.path.join(save_dir, f'run_{run_id}_task_{task_id}_{suffix}_thr_current.npy'), 
                np.array(self.ogc_loss_fn.history_prob_thr_current)) 
        np.save(os.path.join(save_dir, f'run_{run_id}_task_{task_id}_{suffix}_thr_buffer.npy'), 
                np.array(self.ogc_loss_fn.history_prob_thr_buffer)) 
        self._save_constraint_stats(task_id)
    
        if not self.buffer.is_empty(): 
            all_data = self.buffer.get_all_data() 
            labels = all_data[1] 
            if hasattr(self.buffer, 'true_labels'): 
                true_labels = self.buffer.true_labels 
                correct_count = (labels == true_labels).sum().item() 
                purity = correct_count / len(labels) 
                print(f"Task {task_id} Buffer Purity: {purity:.4f} ({correct_count}/{len(labels)})") 
                with open(os.path.join(save_dir, f'purity_{run_id}_{suffix}.txt'), 'a') as f: 
                    f.write(f"Task {task_id}: {purity:.4f}\n") 
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch, true_labels=None):
        ...
        with torch.no_grad():
            ...
            # Trigger GMM data saving for visualization in Task 3 or 4
            save_gmm_path = None
            if self._current_task in [2, 3] and epoch == (self.args.n_epochs - 1):
                save_dir = self._get_mode_output_dir("/home/supermicro/LJW/mam/mammoth/output/gmm_plots/")
                suffix = self._get_plot_mode_suffix()
                run_id = self.args.ogc_run_id
                save_gmm_path = os.path.join(save_dir, f"run_{run_id}_task_{self._current_task}_{suffix}_current.npy")
            ...
            if getattr(self.args, 'disable_dynamic_threshold', False):
                ...
                prob_thr_t, tau_t = self.ogc_loss_fn.get_tau_t(current_lr, is_buffer=False, epsilon_mult=epsilon_mult, save_gmm_path=save_gmm_path)
            ...
            if true_labels is not None:
                ...
                self._update_constraint_stats(
                    'current',
                    noisy_mask_current,
                    low_conf_mask_current,
                    clipped_mask_current
                )
        ...
        combined_loss_current = ce_loss_current.mean() * (1 - current_ogc_weight) + \
                        ogc_loss_current * current_ogc_weight
        ...
        if not self.buffer.is_empty():
            ...
            if not self.args.use_aer or self.is_aer_fitting_epoch(epoch):
                ...
                with torch.no_grad():
                    ...
                    # Trigger GMM data saving for buffer visualization in Task 3 or 4
                    save_gmm_path_buf = None
                    if self._current_task in [2, 3] and epoch == (self.args.n_epochs - 1):
                        save_dir = self._get_mode_output_dir("/home/supermicro/LJW/mam/mammoth/output/gmm_plots/")
                        suffix = self._get_plot_mode_suffix()
                        run_id = self.args.ogc_run_id
                        save_gmm_path_buf = os.path.join(save_dir, f"run_{run_id}_task_{self._current_task}_{suffix}_buffer.npy")
                    ...
                    if getattr(self.args, 'disable_dynamic_threshold', False):
                        ...
                        prob_thr_t, tau_t = self.ogc_loss_fn.get_tau_t(current_lr, is_buffer=True, save_gmm_path=save_gmm_path_buf)
