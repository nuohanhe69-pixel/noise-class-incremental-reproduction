    ...
    def load_data(self):
        ...
        if not os.path.exists(self.data_path):
            raise FileNotFoundError(
                f"NTU60 data file not found: {self.data_path}. "
                "Expected an .npz file with x_train/y_train/x_test/y_test arrays."
            )

        npz_data = np.load(self.data_path)
        required_keys = {'x_train', 'y_train', 'x_test', 'y_test'}
        missing_keys = required_keys.difference(npz_data.files)
        if missing_keys:
            raise KeyError(f"NTU60 .npz is missing keys: {sorted(missing_keys)}")
        ...
        valid_frame_num = np.maximum(valid_frame_num, 1)
...
from datasets.__init__ import register_dataset

@register_dataset('seq-ntu60')
class SequentialNtu60(ContinualDataset):
    ...
    def __init__(
        self,
        args,
        data_path: str = 'data/NTU60_CS.npz',
        num_channels: int = 3,
        num_joints: int = 25,
        num_frames: int = 120,
        num_skeletons: int = 2,
        ...
        self.args.num_joints = num_joints
        self.args.num_frames = num_frames
        self.args.num_skeletons = num_skeletons
        self.SIZE = (num_channels, num_frames, num_joints, num_skeletons)
        
        self.train_dataset = Ntu60(self.args.data_path, split="train",
                                   p_interval=[0.5, 1], window_size=self.args.num_frames,
                                   transform=self.TRANSFORM)

        self.test_dataset = Ntu60(self.args.data_path, split="test",
                                  p_interval=[0.95], window_size=self.args.num_frames,
    ...
    @property
    def num_frames(self):
        return self.args.num_frames

    @property
    def num_channels(self):
        return self.args.num_channels

    @property
    def num_joints(self):
        return self.args.num_joints
    ...
    def get_normalization_transform():
        return transforms.Compose([ntu_to_tensor()])
    ...
    def get_class_names(self):
        if self.class_names is None:
            self.class_names = [f"ntu60_action_{idx:02d}" for idx in range(self.N_CLASSES)]
